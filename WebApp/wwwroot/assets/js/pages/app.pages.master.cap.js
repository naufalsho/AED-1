...
function initDt(response) {
    if (!$.fn.DataTable.isDataTable(dTable)) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (isAllowDelete()) {
                            delBtn = '<a href="' + thisUrl + '/' + data + '" type="button" class="btn btn-outline-danger btn-sm btn-delete" data-type="Cap" data-detail="name <b>' + row.name + '</b>">Delete</a>';
                        }
        ...
        if (isAllowCreate()) {
            $('#dtBtn').append('<a href="' + thisUrl + '/create" class="btn btn-outline-primary btn-modal">Add Cap</a>');
        }
