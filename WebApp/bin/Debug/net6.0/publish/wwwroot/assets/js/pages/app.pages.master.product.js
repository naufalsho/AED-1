...
function getDataParent() {
    ...
    $.get(thisUrl + '/brands').done(function (response) {
...
function initDtGeneral(response) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (isAllowDelete()) {
                        delBtn = '<a href="' + thisUrl + '/type/' + data + '" type="button" class="btn btn-outline-danger btn-sm btn-delete" data-type="Product Type" data-detail="name <b>' + row.name + '</b>">Delete</a>';
                    }
    ...
    if (isAllowCreate()) {
        $('#dtBtnAddGeneral').append('<a href="' + thisUrl + '/type/create" class="btn btn-outline-primary btn-modal">Add Product Type</a>');
    }
}
...
function initDtParent(response) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (isAllowEdit()) {
                        editBtn = '<a href="' + thisUrl + '/brand/' + data + '" class="btn btn-outline-warning btn-sm me-2 btn-modal">Edit</a>';
                    }

                    if (isAllowDelete()) {
                        delBtn = '<a href="' + thisUrl + '/brand/' + data + '" type="button" class="btn btn-outline-danger btn-sm btn-delete" data-type="Product Brand" data-detail="name <b>' + row.name + '</b>">Delete</a>';
                    }
    ...
    if (isAllowCreate()) {
        $('#dtBtnAddParent').append('<a href="' + thisUrl + '/brand/create" class="btn btn-outline-primary btn-modal">Add Product Brand</a>');
    }
}
